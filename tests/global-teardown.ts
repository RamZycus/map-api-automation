...
async function globalTeardown(config: FullConfig) {
  ...
  if (fs.existsSync(resultsFile)) {
    try {
      ...
      const totalTests = results.suites.reduce((sum: number, suite: any) => sum + suite.specs.length, 0);
      const passedTests = results.suites.reduce((sum: number, suite: any) => 
        sum + suite.specs.filter((spec: any) => spec.ok).length, 0);
      const failedTests = totalTests - passedTests;
      
      const totalDuration = results.suites.reduce((sum: number, suite: any) => sum + suite.duration, 0);
